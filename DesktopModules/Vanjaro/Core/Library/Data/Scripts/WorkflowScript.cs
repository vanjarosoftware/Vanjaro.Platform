...
namespace Vanjaro.Core.Data.Scripts
{
    public class WorkflowScript
    {
        ...
        {
            Sql sb = Sql.Builder.Append("SELECT p.ID,p.TabID,p.Version,p.StateID,p.IsPublished,t.TabName,t.Title,t.Description");
        ...
        internal static Sql GetPagesByUserID(int UserID, int Page, int PageSize, int StateID)
        {
            Sql sb = Sql.Builder.Append("select * from (SELECT ROW_NUMBER() OVER (ORDER BY p.ID desc) AS Row_Number, p.ID,p.TabID,p.Version,p.StateID,p.IsPublished,t.TabName,t.Title,t.Description,ps.Name as State,w.Name as Workflow");
            sb.Append("FROM  " + CommonScript.TablePrefix + "vj_core_pages p JOIN (SELECT tabid, Max(version) AS Version FROM   " + CommonScript.TablePrefix + "vj_core_pages WHERE  ispublished = 0 GROUP  BY tabid) p_LatestVersion ON ( p.tabid = p_LatestVersion.tabid AND p.version = p_LatestVersion.version ) ");
            sb.Append("LEFT JOIN " + CommonScript.DnnTablePrefix + "tabs t ON p.tabid = t.tabid ");
        ...
        internal static Sql GetPagesCountByUserID(int UserID, int StateID)
        {
            Sql sb = Sql.Builder.Append("SELECT count(*) as Count");
            sb.Append("FROM  " + CommonScript.TablePrefix + "vj_core_pages p JOIN (SELECT tabid, Max(version) AS Version FROM   " + CommonScript.TablePrefix + "vj_core_pages WHERE  ispublished = 0 GROUP  BY tabid) p_LatestVersion ON ( p.tabid = p_LatestVersion.tabid AND p.version = p_LatestVersion.version ) ");
            sb.Append("LEFT JOIN " + CommonScript.DnnTablePrefix + "tabs t ON p.tabid = t.tabid ");
            sb.Append("LEFT JOIN " + CommonScript.TablePrefix + "VJ_Core_WorkflowState as ps on ps.StateID=p.StateID");
            sb.Append("LEFT JOIN (SELECT stateid FROM   " + CommonScript.TablePrefix + "vj_core_workflowstatepermission wsp ");
        ...
        internal static Sql GetStatesforPendingReview(int PortalID, int UserID)
        {
            Sql sb = Sql.Builder.Append("select * from (SELECT p.StateID as Value,( w.Name + ' > '+  ps.Name )as Text");
            sb.Append("FROM  " + CommonScript.TablePrefix + "vj_core_pages p JOIN (SELECT tabid, Max(version) AS Version FROM   dbo.vj_core_pages WHERE  ispublished = 0 GROUP  BY tabid) p_LatestVersion ON ( p.tabid = p_LatestVersion.tabid AND p.version = p_LatestVersion.version ) ");
            sb.Append("LEFT JOIN " + CommonScript.DnnTablePrefix + "tabs t ON p.tabid = t.tabid");
            sb.Append("left join " + CommonScript.TablePrefix + "VJ_Core_WorkflowState as ps on ps.StateID=p.StateID");
            sb.Append("left join " + CommonScript.TablePrefix + "VJ_Core_Workflow as w on w.ID=ps.WorkflowID");
            sb.Append("LEFT JOIN (SELECT stateid FROM   " + CommonScript.TablePrefix + "vj_core_workflowstatepermission wsp ");
