...
namespace Vanjaro.Core.Data.PetaPoco
{
    ...
    public class Database : IDisposable, IDatabase
    {
        ...
        {
            using (IDbConnection conn = _sharedConnection ?? _factory.CreateConnection())
        ...
        private void AddParam(IDbCommand cmd, object item, string ParameterPrefix)
        {
            ...
            {
                Func<object, object> fn = Database.Mapper.GetToDbConverter(item.GetType());
                if (fn != null)
                {
                    item = fn(item);
                }
        ...
        private string AddSelectClause<T>(string sql)
        {
            ...
            if (!rxSelect.IsMatch(sql))
            {
                ...
                string tableName = EscapeTableName(pd.TableInfo.TableName);
        ...
        private static readonly Regex rxColumns = new Regex(@"\A\s*SELECT\s+((?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|.)*?)(?<!,\s+)\bFROM\b", RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
        private static readonly Regex rxOrderBy = new Regex(@"\bORDER\s+BY\s+(?!.*?(?:\)|\s+)AS\s)(?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|[\w\(\)\.])+(?:\s+(?:ASC|DESC))?(?:\s*,\s*(?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|[\w\(\)\.])+(?:\s+(?:ASC|DESC))?)*", RegexOptions.RightToLeft | RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
        private static readonly Regex rxDistinct = new Regex(@"\ADISTINCT\s", RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
        ...
        public Dictionary<TKey, TValue> Dictionary<TKey, TValue>(string sql, params object[] args)
        {
            ...
            foreach (Dictionary<string, object> line in Query<Dictionary<string, object>>(sql, args))
            {
        ...
        private IEnumerable<T> Query<T>(T instance, Sql Sql)
        {
            ...
            try
            {
                using (IDbCommand cmd = CreateCommand(_sharedConnection, sql, args))
                {
                    ...
                    using (r)
                    {
                        Func<IDataReader, T, T> factory = pd.GetFactory(cmd.CommandText, _sharedConnection.ConnectionString, ForceDateTimesToUtc, 0, r.FieldCount, r, instance) as Func<IDataReader, T, T>;
                        while (true)
                        {
                            T poco;
                            try
        ...
        private object GetAutoMapper(Type[] types)
        {
            ...
            try
            {
                ...
                // Cache it
                Delegate del = m.CreateDelegate(Expression.GetFuncType(types.Concat(types.Take(1)).ToArray()));
        ...
        private Func<IDataReader, object, TRet> CreateMultiPocoFactory<TRet>(Type[] types, string sql, IDataReader r)
        {
            ...
            for (int i = 0; i < types.Length; i++)
            {
                ...
                Delegate del = FindSplitPoint(types[i], i + 1 < types.Length ? types[i + 1] : null, sql, r, ref pos);
                dels.Add(del);
        ...
        private Func<IDataReader, object, TRet> GetMultiPocoFactory<TRet>(Type[] types, string sql, IDataReader r)
        {
            ...
            try
            {
                ...
                // Create the factory
                Func<IDataReader, object, TRet> Factory = CreateMultiPocoFactory<TRet>(types, sql, r);
        ...
        public IEnumerable<TRet> Query<TRet>(Type[] types, object cb, Sql sql)
        {
            ...
            try
            {
                using (IDbCommand cmd = CreateCommand(_sharedConnection, sql.SQL, sql.Arguments))
                {
                    IDataReader r;
                    try
                    {
                    ...
                    Func<IDataReader, object, TRet> factory = GetMultiPocoFactory<TRet>(types, sql.SQL, r);
                    if (cb == null)
                    {
                ...
                using (IDbCommand cmd = CreateCommand(_sharedConnection, sql, args))
                {
                    ...
                    using (r)
                    {
                        ...
                        do
                        {
                            ...
                            PocoData pd = PocoData.ForType(types[typeIndex - 1]);
                            Delegate factory = pd.GetFactory(cmd.CommandText, _sharedConnection.ConnectionString, ForceDateTimesToUtc, 0, r.FieldCount, r, null);
        ...
        {
            int index = 0;
            Dictionary<string, object> primaryKeyValuePairs = GetPrimaryKeyValues(PocoData.ForType(typeof(T)).TableInfo.PrimaryKey, primaryKey);
        ...
        public int Update(string tableName, string primaryKeyName, object poco, object primaryKeyValue, IEnumerable<string> columns)
        {
            ...
            Dictionary<string, object> primaryKeyValuePairs = GetPrimaryKeyValues(primaryKeyName, primaryKeyValue);
            ...
            string sql = string.Format("UPDATE {0} SET {1} WHERE {2}", EscapeTableName(tableName), sb, BuildPrimaryKeySql(primaryKeyValuePairs, ref index));
            ...
            int result = Execute(sql, rawvalues.ToArray());
        ...
        public int Delete(string tableName, string primaryKeyName, object poco, object primaryKeyValue)
        {
            ...
            // Do it
            int index = 0;
            string sql = string.Format("DELETE FROM {0} WHERE {1}", EscapeTableName(tableName), BuildPrimaryKeySql(primaryKeyValuePairs, ref index));
        ...
        public class PocoData
        {
            ...
            public static PocoData ForObject(object o, string primaryKeyName)
            {
                ...
                if (t == typeof(System.Dynamic.ExpandoObject))
                {
                    ...
                    foreach (string col in ((IDictionary<string, object>)o).Keys)
                    {
            ...
            {
                // Check cache
                string key = string.Format("{0}:{1}:{2}:{3}:{4}:{5}", sql, connString, ForceDateTimesToUtc, firstColumn, countColumns, instance != GetDefault(type));
                RWLock.EnterReadLock();
                try
                {
                ...
                try
                {
                    ...
                    // Create the method
                    DynamicMethod m = new DynamicMethod("petapoco_factory_" + PocoFactories.Count.ToString(), type, new Type[] { typeof(IDataReader), type }, true);
                    ILGenerator il = m.GetILGenerator();
                        ...
                        if (type.IsValueType || type == typeof(string) || type == typeof(byte[]))
                    {
                        ...
                        Func<object, object> converter = GetConverter(ForceDateTimesToUtc, null, srcType, type);
                    ...
                    else if (type == typeof(Dictionary<string, object>))
                    {
                        ...
                        Type delegateType = typeof(Func<,,>).MakeGenericType(typeof(IDataReader), type, typeof(Dictionary<string, object>));
                        Delegate localDel = Delegate.CreateDelegate(delegateType, func.Target, func.Method);
                    ...
                    else if (type == typeof(object[]))
                    {
                        ...
                        {
                            object[] obj = new object[countColumns - firstColumn];
                            for (int i = firstColumn; i < firstColumn + countColumns; i++)
                            {
                                object value = reader.IsDBNull(i) ? null : reader.GetValue(i);
                        ...
                        Type delegateType = typeof(Func<,,>).MakeGenericType(typeof(IDataReader), type, typeof(object[]));
                        Delegate localDel = Delegate.CreateDelegate(delegateType, func.Target, func.Method);
                        PocoFactories.Add(key, localDel);
                        return localDel;
                    }
                    else
                    {
                        ...
                        for (int i = firstColumn; i < firstColumn + countColumns; i++)
                        {
                            ...
                            if (!Columns.TryGetValue(r.GetName(i), out PocoColumn pc) && !Columns.TryGetValue(r.GetName(i).Replace("_", ""), out pc))
                            {
                                continue;
                            }
                            ...
                            // Do we need to install a converter?
                            Func<object, object> converter = GetConverter(ForceDateTimesToUtc, pc, srcType, dstType);
                    ...
                    // Cache it, return it
                    Delegate del = m.CreateDelegate(Expression.GetFuncType(typeof(IDataReader), type, type));
            ...
            public static Func<object, object> GetConverter(bool forceDateTimesToUtc, PocoColumn pc, Type srcType, Type dstType)
            {
                ...
                if (Database.Mapper != null)
                {
                    ...
                    {
                        IMapper2 m2 = Database.Mapper as IMapper2;
                        if (m2 != null)
                        {
        ...
        private class EnumMapper : IDisposable
        {
    ...
    public class Sql
    {
        ...
        private void Build(StringBuilder sb, List<object> args, Sql lhs)
        {
            if (!string.IsNullOrEmpty(_sql))
            {
                ...
                string sql = Database.ProcessParams(_sql, _args, args);
    ...
    public class SqlBuilder
    {
        ...
        public class Template
        {
            ...
            private readonly List<object> finalParams = new List<object>();
            private int dataSeq;
            ...
            private void ResolveSql()
            {
                ...
                {
                    foreach (KeyValuePair<string, Clauses> pair in builder.data)
                    {
            ...
            {
                foreach (KeyValuePair<string, string> pair in builder.defaultsIfEmpty)
                {
