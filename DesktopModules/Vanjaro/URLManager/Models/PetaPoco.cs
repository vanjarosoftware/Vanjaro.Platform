...
namespace Vanjaro.URL.PetaPoco
{
    ...
    public class Database : IDisposable, IDatabase
    {
        ...
        static Regex rxColumns = new Regex(@"\A\s*SELECT\s+((?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|.)*?)(?<!,\s+)\bFROM\b", RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
        static Regex rxOrderBy = new Regex(@"\bORDER\s+BY\s+(?!.*?(?:\)|\s+)AS\s)(?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|[\w\(\)\.])+(?:\s+(?:ASC|DESC))?(?:\s*,\s*(?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|[\w\(\)\.])+(?:\s+(?:ASC|DESC))?)*", RegexOptions.RightToLeft | RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
        static Regex rxDistinct = new Regex(@"\ADISTINCT\s", RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
...
//            // Store factory and connection string
//            _connectionString = Config.GetConnectionString();//ConfigurationManager.ConnectionStrings[connectionStringName].ConnectionString;
//            _providerName = providerName;
//            CommonConstruct();
//        }
...
//                TableInfo.TableName = CommonScript.TablePrefix + TableInfo.TableName;
