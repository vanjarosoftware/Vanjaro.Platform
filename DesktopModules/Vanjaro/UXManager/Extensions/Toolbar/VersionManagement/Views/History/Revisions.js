app.controller('history_revisions', function ($scope, $attrs, $http, CommonSvc, SweetAlert) {
    ...
    $scope.LoadVersion = function () {
        common.webApi.get('Revisions/GetDate', 'Locale=' + window.parent.VJLocal).success(function (data) {
            if (data != null) {
                $scope.ui.data = data;
    ...
    $scope.Click_ViewLogs = function (Data) {
        if (Data.IsLogsExist)
            window.parent.OpenPopUp(null, 600, 'right', 'Logs', window.parent.CurrentExtTabUrl + '&guid=33d8efed-0f1d-471e-80a4-6a7f10e87a42#moderator?version=' + Data.Version + '&entity=VJPage&entityid=' + Data.TabID);
    };
