    ...
    public class SettingController : UIEngineController
    {
        ...
        public string Delete(dynamic Data)
        {
            if (bool.Parse(Data.ToString()))
                HostController.Instance.UpdateEncryptedString("Vanjaro.Integration.YouTube.ApiKey", string.Empty, Config.GetDecryptionkey());
            else
                PortalController.UpdateEncryptedString(PortalSettings.PortalId, "Vanjaro.Integration.YouTube.ApiKey", string.Empty, Config.GetDecryptionkey());
            return string.Empty;
        }
