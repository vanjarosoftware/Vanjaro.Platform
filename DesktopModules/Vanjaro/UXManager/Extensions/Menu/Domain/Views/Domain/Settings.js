app.controller('domain_settings', function ($scope, $routeParams, CommonSvc, SweetAlert) {
    ...
    $scope.Click_Save = function () {
        if (mnValidationService.DoValidationAndSubmit('', 'domain_settings')) {
            ...
            common.webApi.post('domain/AddUpdateSiteAlias', '', $scope.ui.data.UpdateSiteAliasRequest.Options).success(function (Response) {
                if (Response.IsSuccess) {
                    ...
                    if (ParentScope != undefined && ParentScope.element(".menuextension").scope() != undefined && ParentScope.element(".menuextension").scope().PortalAliases != undefined) {
                        ParentScope.element(".menuextension").scope().PortalAliases = Response.Data.SiteAliases.PortalAliases;
                    ...
                    window.parent.ShowNotification($scope.ui.data.UpdateSiteAliasRequest.Options.HTTPAlias, Response.Message, 'error');
                }
            });
        }
    };
