...
namespace Vanjaro.UXManager.Extensions.Menu.Security
{
    public static partial class Managers
    {
        public class SecurityManager
        {
            ...
            public static dynamic GetBasicLoginSettings(PortalSettings portalSettings)
            {
                ...
                try
                {
                    ...
                    response = new
                    {
                        ...
                        UpdateBasicLoginSettingsRequest = settings//MapUpdateBasicLoginSettings(settings)
                    };
                }
                catch (Exception exc)
                {
            ...
            //private static dynamic MapUpdateBasicLoginSettings(dynamic settings)
            //{
            //    UpdateBasicLoginSettingsRequest updateBasicLoginSettingsRequest = new  UpdateBasicLoginSettingsRequest()
            //    {
            //        CaptchaChangePassword = settings.CaptchaChangePassword,
            //        CaptchaLogin = settings.CaptchaChangePassword,
            //        CaptchaRetrivePassword = settings.CaptchaRetrivePassword,
            //        CultureCode = settings.CultureCode,
            //        DefaultAuthProvider = settings.DefaultAuthProvider,
            //        HideLoginControl = settings.HideLoginControl,
            //        PrimaryAdministratorId = settings.PrimaryAdministratorId,
            //        RedirectAfterLoginTabId = settings.RedirectAfterLoginTabId,
            //        RedirectAfterLogoutTabId = settings.RedirectAfterLogoutTabId,
            //        RequireValidProfileAtLogin = settings.RequireValidProfileAtLogin
            //    };

            //    return updateBasicLoginSettingsRequest;
            //}
            #endregion
            #region Get Registration Settings
            public static dynamic GetRegistrationSettings(PortalSettings portalSettings)
            {
                List<KeyValuePair<string, int>> userRegistrationOptions = Managers.SecurityManager.GetUserRegistrationOptions();
                List<KeyValuePair<string, int>> registrationFormTypeOptions = Managers.SecurityManager.GetRegistrationFormOptions();
                var Results = new
                {
                    Settings = new
                    {
